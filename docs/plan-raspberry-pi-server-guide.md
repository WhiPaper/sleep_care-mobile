...
> 참고: 앱 개발자 모드에는 Pi 개발 테스트 도구가 있다. WSS 서버만 먼저 떠 있으면 QR/Avahi 없이도 직접 endpoint로 SPKI, hello, Eye-only 세션, Synthetic HR, 종료까지 확인할 수 있으므로 [Pi 개발자 디버그 가이드](./pi-developer-debug-guide.md)를 함께 본다.
